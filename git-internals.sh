...
# Commit
run cat -A .git/objects/43/36aafb9d1d3e86e993bf313d274ac4026ec16b
run 'cat .git/objects/43/36aafb9d1d3e86e993bf313d274ac4026ec16b | zlib-uncompress | cat -A'
show commit

# Tree
run 'cat .git/objects/b8/04fb4dc50438095ead879326444454d442901c | zlib-uncompress | cat -A'
run 'cat .git/objects/b8/04fb4dc50438095ead879326444454d442901c | zlib-uncompress | xxd -p -s 23 -l 20'
run 'cat .git/objects/b8/04fb4dc50438095ead879326444454d442901c | zlib-uncompress | xxd -p -s 56 -l 20'
fake_run 'cat .git/objects/b8/04fb4dc50438095ead879326444454d442901c | zlib-uncompress | decode' 'tree 71^@100644 CdL.txt^@34786b0c4f9a77a533adcdb742b62870269e402940000 folder^@49b4e4747c1c1bd8746235f146fe287992add5e9'
...
# Blob
run 'cat .git/objects/34/786b0c4f9a77a533adcdb742b62870269e4029 | zlib-uncompress | cat -A'
...
# Create blobs
run 'BLOB="Capitole du libre 2016\n" && echo -ne $BLOB | cat -A'
run 'GIT_OBJECT=$(echo -n "blob $(echo -ne $BLOB | wc --bytes)\x00$BLOB") && echo -ne $GIT_OBJECT | cat -A'
run 'BLOB_CDL_SHA1=$(echo -ne $GIT_OBJECT | sha1sum | awk "{ print \$1 }") && echo $BLOB_CDL_SHA1'
run 'mkdir -p .git/objects/${BLOB_CDL_SHA1:0:2} && echo -ne $GIT_OBJECT | zlib-compress > .git/objects/${BLOB_CDL_SHA1:0:2}/${BLOB_CDL_SHA1:2}'
...
run '(echo -ne "100644 CdL.txt\x00"; echo $BLOB_CDL_SHA1 | xxd -r -p; echo -ne "40000 folder\x00"; echo $TREE_FOLDER_SHA1 | xxd -r -p) >/tmp/tree && cat -A /tmp/tree && echo
...
# Create commit
run 'COMMIT_DATE=$(date -d "2016-11-19T17:00:00+0100" +"%s %z") && echo $COMMIT_DATE'
run 'COMMIT="tree $TREE_ROOT_SHA1\nauthor Alexandre Garnier <zigarn@gmail.com> $COMMIT_DATE\ncommitter Alexandre Garnier <zigarn@gmail.com> $COMMIT_DATE\n\n1st commit\n" && echo -ne $COMMIT | cat -A'
...
# GIT_COMMITTER_NAME='Alexandre Garnier' \
# GIT_COMMITTER_EMAIL='zigarn@gmail.com' \
# GIT_COMMITTER_DATE='2016-11-19T17:30:00+0100' \
# git -c core.commentchar=- tag --annotate \
#     --message "Entrer dans les entrailles de Git, ou comment faire un commit sans faire du Git.
#
# #cdl2016
...
#       GIT_COMMITTER_NAME='Alexandre Garnier' \
#       GIT_COMMITTER_EMAIL='zigarn@gmail.com' \
#       GIT_AUTHOR_DATE='2016-11-19T17:30:00+0100' \
...
cd $BASE_DIR
run git show --format=fuller conclusion-cdl2016
